...
class ChessCom:
	...
	def get_titled_players(self, title_abbrev: str) -> dict:
		return get(
	...
	def check_player_status(self, username: str) -> dict:
		return get(
			f"{self.api}/pub/player/{username}/is-online",
			headers=self.headers).json()

	def get_player_current_games(self, username: str) -> dict:
		return get(
			f"{self.api}/pub/player/{username}/games",
			headers=self.headers).json()

	def get_player_turns(self, username: str) -> dict:
		return get(
			f"{self.api}/pub/player/{username}/games/to-move",
			headers=self.headers).json()

	def get_player_monthly_archives(self, username: str) -> dict:
		return get(
			f"{self.api}/pub/player/{username}/games/archives",
			headers=self.headers).json()

	def get_player_old_games(self, username: str, year: int, month: int) -> dict:
		return get(
			f"{self.api}/pub/player/{username}/games/{year}/{month}",
			headers=self.headers).json()

	def get_player_clubs(self, username: str) -> dict:
		return get(
			f"{self.api}/pub/player/{username}/clubs",
			headers=self.headers).json()

	def get_player_matches(self, username: str) -> dict:
		return get(
			f"{self.api}/pub/player/{username}/matches",
			headers=self.headers).json()

	def get_player_tournaments(self, username: str) -> dict:
		return get(
			f"{self.api}/pub/player/{username}/tournamens",
			headers=self.headers).json()

	def get_club_info(self, url_id: str) -> dict:
		return get(
			f"{self.api}/pub/club/{url_id}",
			headers=self.headers).json()

	def get_club_members(self, url_id: str) -> dict:
		return get(
			f"{self.api}/pub/club/{url_id}/members",
			headers=self.headers).json()

	def get_club_matches(self, url_id: str) -> dict:
		return get(
			f"{self.api}/pub/club/{url_id}/matches",
			headers=self.headers).json()

	def get_tournament_info(self, url_id: str) -> dict:
		return get(
	...
	def get_daily_team_matches(self, id: int) -> dict:
		return get(
			f"{self.api}/pub/match/{id}",
			headers=self.headers).json()

	def get_daily_team_match_board(
			self,
			id: int,
			board: int = 1) -> dict:
		return get(
	...
	def get_live_team_matches(self, id: int) -> dict:
		return get(
			f"{self.api}/pub/match/live/{id}",
			headers=self.headers).json()

	def get_live_team_match_board(
			self,
			id: int,
			board: int = 1) -> dict:
		return get(
	...
	def get_country_info(self, iso: str) -> dict:
		return get(
			f"{self.api}/pub/country/{iso}",
			headers=self.headers).json()

	def geT_country_players(self, iso: str) -> dict:
		return get(
			f"{self.api}/pub/country/{iso}/players",
			headers=self.headers).json()

	def get_country_clubs(self, iso: str) -> dict:
		return get(
	...
	def get_daily_puzzle(self) -> dict:
		return get(
			f"{self.api}/pub/puzzle",
			headers=self.headers).json()

	def get_random_daily_puzzle(self) -> dict:
		return get(
			f"{self.api}/pub/puzzle/random",
			headers=self.headers).json()

	def get_streamers_info(self) -> dict:
		return get(
			f"{self.api}/pub/streamers",
			headers=self.headers).json()

	def get_leaderboards(self) -> dict:
		return get(
			f"{self.api}/pub/leaderboards",
			headers=self.headers).json()
